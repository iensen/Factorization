...
public class FactorizationAlgorithmsTests extends
    ...
    public void testEP1() {
    	...
    	int[] algorithms = new int[] { 1, 2, 1, 1, 2 };
    	runInput(inputs, algorithms);
    	assertTrue("Could not find the dialog!", solo.searchText("You should input numbers into all the edit boxes"));
    }
	...
	private void assertSortedEqual(ArrayList<BigInteger>[] results,ArrayList<BigInteger>[] expectedOutput) {
	...
	private ArrayList<BigInteger>[] runInput(BigInteger[] inputs,
		...
		for (int i = 0; i < algorithmUserIds.length; i++) {
			...
			solo.clickOnView(solo.getView(TextView.class, algorithmUserIds[i]-1));
		}
